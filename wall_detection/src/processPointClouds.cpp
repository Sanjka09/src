...
template<typename PointT>
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::RANSAC_PlaneSegment_with_init_Scratch(typename pcl::PointCloud<PointT>::Ptr cloud, pcl::ModelCoefficients::Ptr plane_coeff, int maxIterations, float disttanceThreshold)
{
    // Time segmentation process
    auto startTime = std::chrono::steady_clock::now();
...
template<typename PointT>
std::pair<line_data, std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr>> ProcessPointClouds<PointT>::RANSAC_LineSegment_Scratch(typename pcl::PointCloud<PointT>::Ptr cloud, int maxIterations, float distanceThreshold, double robot_theta)
{
    // Time segmentation process
    auto startTime = std::chrono::steady_clock::now();
    ...
    std::cout<< "robot theta is: "<< robot_theta<< std::endl;
    for(int it = 0; it < maxIterations; it++)
    {
        std::unordered_set<int> inliers;
      ...
      //  std::cout << " random point owned3" << std::endl;
        ...
        if(inliers.size() > inliersResult.size())
        {
            ...
            float nx=-a*c/sqrt(pow(a, 2)+pow(b, 2));
            float ny=-b*c/sqrt(pow(a, 2)+pow(b, 2));
            line_coeff.angle_x=atan2(nx, ny)/3.14*180+180;
            // line_coeff.angle_x=atan2(line_coeff.a, line_coeff.b)/3.14*180;
            // if(line_coeff.angle_x<0){
            //     line_coeff.angle_x+=180;
            // }
            PointT pe1, pe2;
            pe1.x= cos(robot_theta/180*3.14)*p1.x-sin(robot_theta/180*3.14)*p1.y;
            pe1.y= sin(robot_theta/180*3.14)*p1.x+cos(robot_theta/180*3.14)*p1.y;
            pe2.x= cos(robot_theta/180*3.14)*p2.x-sin(robot_theta/180*3.14)*p2.y;
            pe2.y= sin(robot_theta/180*3.14)*p2.x+cos(robot_theta/180*3.14)*p2.y;     
            std::cout<<"original line coeff: "<< a<< " "<< b<< " "<< c<< std::endl;

            float ra = pe2.y - pe1.y;
            float rb = pe1.x - pe2.x;
            float rc = pe2.x * pe1.y - pe1.x * pe2.y;
            std::cout<<"transformed line coeff: "<< ra<< " "<< rb<< " "<< rc<< std::endl;
            float angle_x;
            float inter_x;
            float inter_y;
            angle_x=atan2(ra, rb)/3.14*180;

            while(angle_x<0){
                angle_x+=180;
            }
            if(angle_x>50 && angle_x<130){
                //right or left
                inter_x=-rc/ra;
                if(inter_x<0){
            ...
            else if (angle_x>140 || angle_x<40){
                //front or back
                inter_y=-rc/rb;
                if(inter_y>0){
