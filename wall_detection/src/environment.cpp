  ...
  for(pcl::PointCloud<pcl::PointXYZI>::Ptr cluster : cloudClusters)
    ...
    while(cloudall->size()>30){
        ...
        //  std::cout<<"8r mur"<<std::endl;
         allResult = pointProcessorI->RANSAC_LineSegment_Scratch(cloudall,200,0.02, robot_theta);
    ...
    for(int i=0; i<line_count; i++){
        std::cout<<i<< "-r line is " << line_o[i].line_stat << " stat, distance: " << line_o[i].line_dist << " angle: "<< line_o[i].angle_x <<std::endl;
    ...
    std_msgs::Int16MultiArray silo_i;
    ...
    if(left_index!=-1){
        ...
        // coordinate_buff.data.push_back(line_o[front_index].line_dist*100);
        coordinate_buff.data.push_back(line_o[right_index].line_dist*100);
        double angle_rx = line_o[right_index].angle_x-180;
        if(angle_rx>360){
            angle_rx=angle_rx-360;
        }
        if(angle_rx<0){
            angle_rx=angle_rx+360;
        }
        coordinate_buff.data.push_back(angle_rx);
    ...
    //intersection calculate right and front;

    float front_inter_x=(line_o[right_index].c*line_o[front_index].b - line_o[front_index].c*line_o[right_index].b)/(line_o[front_index].a*line_o[right_index].b - line_o[right_index].a*line_o[front_index].b);
    float front_inter_y=(line_o[right_index].c*line_o[front_index].a - line_o[front_index].c*line_o[right_index].a)/(line_o[front_index].b*line_o[right_index].a - line_o[right_index].b*line_o[front_index].a);


    float back_inter_x=(line_o[right_index].c*line_o[back_index].b - line_o[back_index].c*line_o[right_index].b)/(line_o[back_index].a*line_o[right_index].b - line_o[right_index].a*line_o[back_index].b);
    float back_inter_y=(line_o[right_index].c*line_o[back_index].a - line_o[back_index].c*line_o[right_index].a)/(line_o[back_index].b*line_o[right_index].a - line_o[right_index].b*line_o[back_index].a);

    float right_inter_y = -(line_o[right_index].c+line_o[right_index].a*0.22)/line_o[right_index].b;
    silo_i.data.push_back(right_inter_y*100);
    if(ongo == "red"){
        if(front_inter_x<0){
            float silo_dist = sqrt(pow((0.22-front_inter_x),2)+pow((right_inter_y-front_inter_y),2))*100;
            silo_i.data.push_back(silo_dist);
        }
        else{
            float silo_dist = -sqrt(pow((0.22-front_inter_x),2)+pow((right_inter_y-front_inter_y),2))*100;
            silo_i.data.push_back(silo_dist);
        ...
        if(back_inter_x<0){
            float silo_dist = -sqrt(pow((0.22-back_inter_x),2)+pow((right_inter_y-back_inter_y),2))*100;
            silo_i.data.push_back(silo_dist);
        }
        else{
            float silo_dist = sqrt(pow((0.22-back_inter_x),2)+pow((right_inter_y-back_inter_y),2))*100;
...
int main (int argc, char** argv)
{
    ...
    silo_index=nh.advertise<std_msgs::Int16MultiArray>("silo_index", 10);
